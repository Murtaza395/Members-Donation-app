<?php
   include "php/dbconnect.php";
    ...
    if(isset($_POST['login'])) {
        ...
        $sql = "SELECT * FROM schools WHERE cnic='".$cnic."' AND password = '".$password."'";
        $q = $conn->query($sql);
        if ($q->num_rows == 1) {
         $res = $q->fetch_assoc();
         if($cnic=$res['cnic']&& $password==$res['password'] && $res['status'] == 'Active') {
            session_start();
            $_SESSION['rainbow_uid'] = $res['id'];
            header("Location: php/checklogin.php");
            exit();
         }
        elseif ($res['cnic']==$cnic && $res['password']==$password && $res['status']=='pending' ) {
...
<html>
    ...
    <form action="login.php" method="post">
        ...
        <input type="text" class="form-control"  data-inputmask="'mask': '99999-9999999-9'"  placeholder="XXXXX-XXXXXXX-X"  name="cnic" />
        <script>
            $(":input").inputmask();
        </script>
        <label for="password">
